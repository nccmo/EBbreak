...
def assemble_seq_sga(readid2seq, junc_seq, tmp_file_path):
    ...
    sga_input = tmp_file_path + ".tmp3.assemble_input.fa"
...
def generate_contig(input_file, output_file, tumor_bp_file, tumor_bam, reference_genome, min_contig_length):
    ...
    with open(input_file, 'r') as hin:
        for line in hin:
            ...
            if len(short_contig_fml_asm) > len(long_contig_fml_asm):
                contig_fml_asm = short_contig_fml_asm
            else:
                contig_fml_asm = long_contig_fml_asm
            ...
            if len(short_contig_velvet) > len(long_contig_velvet):
                contig_velvet = short_contig_velvet
            else:
                contig_velvet = long_contig_velvet
            ...
            if len(short_contig_sga) > len(long_contig_sga):
                contig_sga = short_contig_sga
            else:
                contig_sga = long_contig_sga
            #if len(contig_sga) < min_contig_length: continue


            print >> hout, '\t'.join(F) + '\t' + contig_cap3 + '\t' + long_contig_cap3 + '\t' + short_contig_cap3 + '\t' + contig_fml_asm + '\t' + long_contig_fml_asm + '\t' + short_contig_fml_asm  + '\t' + \
                           contig_velvet + '\t' + long_contig_velvet + '\t' + short_contig_velvet + '\t' + contig_sga + '\t' + long_contig_sga + '\t' + short_contig_sga
    ...
    # subprocess.call(["rm", "-rf", output_file + ".tmp3.assemble_output.fq"])
