    ...
    for read in bamfile.fetch():
        ...
        if right_clipping >= min_major_clip_size:
            ...
            print >> hout, '\t'.join([juncChr_current, str(juncPos_current), juncDir_current, juncseq, 
                                      read.qname + ("/1" if flags[6] == "1" else "/2"), str(read.mapq), str(right_clipping)])
        ...
        if left_clipping >= min_major_clip_size:
            ...
            print >> hout, '\t'.join([juncChr_current, str(juncPos_current), juncDir_current, juncseq, 
                                      read.qname + ("/1" if flags[6] == "1" else "/2"), str(read.mapq), str(left_clipping)])
    ...
    with open(input_file, 'r') as hin:
        for line in hin:
            ...
            if tmp_chr != F[0] or int(F[1]) - tmp_pos > check_interval:
                del_list = []
                for key in key2read:
                    print >> hout, key + '\t' + ';'.join(key2read[key]) + '\t' + ';'.join(key2mapq[key]) + '\t' + ';'.join(key2clipsize[key])
                    del_list.append(key)
